...
前端技术栈：`uni-app + Vue 3 + TypeScript`
...
`risk-v0.3.1` 根据地图信息完整度、跨平台评分差异和可选评论 Evidence 生成风险分数、置信度与理由。`taste-v0.1` 则从显式反馈、轻量行为和近期饮食历史形成匿名偏好；两者在领域模型和持久化快照中保持分离，直到 `recommendation-v0.4.1` 排序时才组合。
