...
用户主动点击“深挖一下”时，系统才通过百度 AI 搜索读取 B站、小红书和大众点评的公开索引标题、链接与摘要。它不登录平台、不读取评论区，结果只作为独立弱线索展示，不改变主排序、候选池、TasteProfile 或 reroll。
...
- 外部服务：高德 Place、可选百度 Place、按需百度 AI 搜索
...
数据库、高德、百度和百度 AI 搜索的环境变量见 [`backend/README.md`](backend/README.md)。所有真实密钥只通过环境变量注入，不进入前端代码或仓库。
...
- 百度 AI 搜索深挖依赖公开搜索索引，只能作为辅助线索，不能代表平台完整评价。
